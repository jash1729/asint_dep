sap.ui.define([
    ...
    return Formatter.extend("com.asint.ais.library.utils.ObjectTemplateValueHelp", {

        _baseURI: "",
        _i18n: {},
        _appNamespace: "comasintais",
        _mV4MasterService: {},
        _fnEventForObjectTemplate: null,
        _aPreselectData : [],
         ...
         */
        handleObjectTemplateValueHelp: function (_fnEvent, bTableMode, aPreSelectDataArray) {

            var that = this;
            var sMode = bTableMode ? sap.m.ListMode.MultiSelect : sap.m.ListMode.SingleSelectMaster;

            this._fnEventForObjectTemplate = _fnEvent;
            if(aPreSelectDataArray && aPreSelectDataArray.length > 0){
                this._aPreselectData = aPreSelectDataArray;
            }

            var sName = "com.asint.ais.library.fragment.DialogObjectTemplateValueHelp";

            if (window._objectType === "EQUI") {
                sName = "com.asint.ais.library.fragment.DialogEquipmentTemplateValueHelp";
            } else if (window._objectType === "FLOC") {
                sName = "com.asint.ais.library.fragment.DialogFLOCTemplateValueHelp";
            } else {
                sName = "com.asint.ais.library.fragment.DialogObjectTemplateValueHelp";
            }
            ...
            var fnResetSearchandSelection = function () {
                var oTable = sap.ui.getCore().byId(Fragment.createId(that._appNamespace, "idObjectTemplateTable"));
                ...
                if (oSearch && oSearch.getProperty("placeholder") && oSearch.getProperty("placeholder") == "Search") {
                    oSearch.setValue("");
                    if (window._objectType === "FLOC" || window._objectType === "EQUI") {
                        var oFilterArr = new Filter([
                            new Filter("type", FilterOperator.EQ, window._objectType),
                        ], false);
            ...
            if (!this._oObjectTemplateValueHelpDialog) {
                ...
                Fragment.load({
                    ...
                    var oObjectTemplateModel = new JSONModel({
                        "data": {
                        },
                        "metadata": {
                            "selectionMode": sMode,
                    ...
                    that._oObjectTemplateValueHelpDialog = oValueHelpDialog;

                    that._oObjectTemplateValueHelpDialog.setModel(oObjectTemplateModel, "mObjectTemplateValueHelp");
                    that._oObjectTemplateValueHelpDialog.setModel(that._mV4MasterService, "mV4MasterService");
                    that._oObjectTemplateValueHelpDialog.setModel(that._i18n, "i18n");
                    that._oObjectTemplateValueHelpDialog.open();
                    fnResetSearchandSelection();
                });

            } else {
                that._oObjectTemplateValueHelpDialog.open();
                that._oObjectTemplateValueHelpDialog.getModel("mObjectTemplateValueHelp").setProperty("/metadata/selectedItems", []);
                that._oObjectTemplateValueHelpDialog.getModel("mObjectTemplateValueHelp").setProperty("/metadata/selectedID", []);
                fnResetSearchandSelection();
            }
         ...
         * @param {Object} oEvent 
         */
        onSelectObjectTemplate: function (oEvent) {

            var mObjectTemplateValueHelp = this._oObjectTemplateValueHelpDialog.getModel("mObjectTemplateValueHelp");
            var sSelectionMode = mObjectTemplateValueHelp.getProperty("/metadata/selectionMode");
            var aSelectedItems = mObjectTemplateValueHelp.getProperty("/metadata/selectedItems");
            var aSelectedID = mObjectTemplateValueHelp.getProperty("/metadata/selectedID");
            ...
            mObjectTemplateValueHelp.setProperty("/metadata/selectedItems", aSelectedItems);
            mObjectTemplateValueHelp.setProperty("/metadata/selectedID", aSelectedID);
            mObjectTemplateValueHelp.setProperty("/metadata/selectedCount", aSelectedID.length);

            if (sSelectionMode === sap.m.ListMode.SingleSelectMaster) {
                this.fnReturnData("OBJT");
            }
        ...
        fnReturnData: function (sType) {
            ...
            if (sType === "OBJT") {
                var mObjectTemplateValueHelp = this._oObjectTemplateValueHelpDialog.getModel("mObjectTemplateValueHelp");
                var oEquipmentTable = sap.ui.getCore().byId(Fragment.createId(this._appNamespace, "idObjectTemplateTable"));
                //var aSelectedItem = oEquipmentTable.getSelectedItems();
                var aSelectedItem = mObjectTemplateValueHelp.getProperty("/metadata/selectedItems");
                var oReturn = {
                    status: "finished",
                    selected: []
                };
                ...
                this._fnEventForObjectTemplate(oReturn);
                this._oObjectTemplateValueHelpDialog.close();
        ...
        onSearch: function (oEvent) {
            ...
            if (sQuery) {
                ...
                if (oTable.getId().includes("idObjectTemplateTable")) {
                    aFilter = new Filter([
                        new Filter({ path: "name", operator: FilterOperator.Contains, value1: sQuery, caseSensitive: false }),
                        new Filter({ path: "shortDescription", operator: FilterOperator.Contains, value1: sQuery, caseSensitive: false }),
                        new Filter({ path: "displayId", operator: FilterOperator.Contains, value1: sQuery, caseSensitive: false })
                    ], false);
                }

                var oFinalFilter = {};
                if (window._objectType) {
                    var oTypeFilter = new Filter({
                        path: "type",
                        operator: FilterOperator.EQ,
                        value1: window._objectType,
                        caseSensitive: true
                    });
                    oFinalFilter = new Filter([aFilter, oTypeFilter], true);
                } else {
         ...
         */
        onObjectTemplateValueHelpDialogConfirm: function () {
         ...
         */
        onObjectTemplateValueHelpDialogClose: function () {
            var objtTempTable = sap.ui.getCore().byId(Fragment.createId(this._appNamespace, "idObjectTemplateTable"));
            objtTempTable.removeSelections();
            this._oObjectTemplateValueHelpDialog.close();
            this._fnEventForObjectTemplate({
                status: "closed",
                selected: []
            });
        ...
        onDataReceived: function () {
            var that = this;
            this.fnFetchInlineCount(this, "idObjectTemplateTable", function (sCount) {
                var sTitle = "";

                if (window._objectType === "EQUI") {
                    sTitle = "objectTemplate.valueHelp.table.equipment.title";
                } else if (window._objectType === "FLOC") {
                    sTitle = "objectTemplate.valueHelp.table.floc.title";
                } else {
                    sTitle = "objectTemplate.valueHelp.table.title";
                }
                var sHeader = that._i18n.getResourceBundle().getText(sTitle, [sCount]);
                that._oObjectTemplateValueHelpDialog.getModel("mObjectTemplateValueHelp").setProperty("/metadata/tableHeader", sHeader);
            });
