sap.ui.define([
    "sap/ui/core/Control",
    "com/asint/ais/library/control/GeneralRiskMatrix/RiskMatrixCell"
], function (Control) {

    return Control.extend("com.asint.ais.library.control.GeneralRiskMatrix.RiskMatrix", {
        metadata: {
            properties: {
                rows: {
                    type: "int",
            ...
            aggregations: {
                cells: {
                    type: "com.asint.ais.library.control.GeneralRiskMatrix.RiskMatrixCell",
                    multiple: true,
                    singularName: "cell"
                },
                xAxisData: {
                    type: "com.asint.ais.library.control.GeneralRiskMatrix.RiskXAxis",
                    multiple: true
                },
                yAxisData: {
                    type: "com.asint.ais.library.control.GeneralRiskMatrix.RiskYAxis",
                    multiple: true
                },
                riskLines: {
                    type: "com.asint.ais.library.control.GeneralRiskMatrix.RiskLine",
                    multiple: true,
                    singularName: "riskLine"
                },
                plotPoints: {
                    type: "com.asint.ais.library.control.GeneralRiskMatrix.PlotPoint",
                    multiple: true,
                    singularName: "plotPoint"
                },
                pointLabels: {
                    type: "com.asint.ais.library.control.GeneralRiskMatrix.RiskMatrixLabel",
                    multiple: true,
        ...
        fnBuildMatrixGrid: function () {
            ...
            for (i = 0.3; i < this.getRows() + 0.3; i += 1.00) {
                for (j = jXOF; j <= this.getCols() + jXLim; j += 1.00) {
                    sHtml += "<rect x=\"" + (j * 100) + "\" y=\"" + (i * 100) + "\" height=\"100\" width=\"100\" stroke=\"black\" stroke-width=\"2\" stroke-opacity=\"1\" />";
                    sHtml += "<text x=\"" + ((j * 100) + 50) + "\" y=\"" + ((i * 100) + 50) + "\" font-family=\"Arial, sans-serif\" class=\"matrixPlotValueText\" font-size=\"24px\" fill=\"black\" text-anchor=\"middle\" dominant-baseline=\"central\"></text>";
                }
            }
        ...
        fnBuildXAxisPoints: function (iAxis) {	
            ...
            if (oPointData.length > 0) {
                var range = oPointData[0].getHigh() - oPointData[1].getHigh();
                if (range >= 0) {
                    order =  "des"
                }else {
                    order =  "asc"
            ...
            if (bRange) {
                for (i = 0; i < oPointData.length; ++i) {
                    var sLow = this.fnAbbreviateNumber(oPointData[i].getLow(), 3),
                        sHigh = this.fnAbbreviateNumber(oPointData[i].getHigh(), 3);

                    if(order === "des"){
                        aPoints.push(sHigh + "-" + sLow);
                    }else {
            ...
            for (i = 0; i < aPoints.length; ++i) {
                ...
                if (i < aMidPoints.length && (!bRange || (bRange && bText))) {
                    ...
                    for (j = 0; j < aWords.length; ++j) {
                        sHtml += "<text text-anchor=\"middle\" class=\"asIntRbiRMXSubLabelPoint\" x=\"" + ((i + iXMOF) * 100) + "\"" +
        ...
        fnBuildYAxisPoints: function (iAxis) {
            ...
            for (i = 0; i < aPoints.length; ++i) {
                ...
                if (i < aMidPoints.length && (!bRange || (bRange && bText))) {
                    ...
                    for (j = 0; j < aWords.length; ++j) {
                        sHtml += "<text text-anchor=\"end\" class=\"asIntRbiRMYSubLabelPoint sapThemeText\" x=\"" + iMidPointXVal + "\"" +
        ...
        fnPlotPoints: function (iScrX, iScrY, iXAxis, iYAxis, iStepSize, bUseAxisLabel) {
            ...
            if(oXAxisData && oYAxisData) {
                ...
                var oXPoints = oXAxisData.getAggregation("axisValues"),
                    sTempScrX = iScrX;
                var xOrder = "";
                if (oXPoints.length > 0) {
                    var range = oXPoints[0].getHigh() - oXPoints[1].getHigh();
                    if (range >= 0) {
                        xOrder = "des"
                    } else {
                        xOrder = "asc"
                ...
                if (xOrder === "asc") {
                    ...
                    if (iScrX === "undefined" || isNaN(iScrX)) {
                        sTempScrX = oXPoints[oXPoints.length - 1].getLow();
                    }
                    if (iScrX > oXPoints[0].getHigh()) {
                        sTempScrX = oXPoints[0].getHigh();
                    } else if (iScrX < oXPoints[oXPoints.length - 1].getLow()) {
                        sTempScrX = oXPoints[oXPoints.length - 1].getLow();
                    }
                    $.each(oXAxisData.getAggregation("axisValues"), function (iDx, oValue) {
                        if (sTempScrX >= oValue.getLow() && sTempScrX <= oValue.getHigh()) {
                            aBox[0] = iDx;
                        }
                    });
                    // var iXRange = oXPoints[aBox[0]].getHigh() - oXPoints[aBox[0]].getLow();
                ...
                var oYPoints = oYAxisData.getAggregation("axisValues").reverse(),
                    sTempScrY = iScrY;
                var yOrder = "";
                if (oYPoints.length > 0) {
                    var rangeY = oYPoints[0].getHigh() - oYPoints[1].getHigh();
                    if (rangeY <= 0) {
                        yOrder = "des"
                    } else {
                        yOrder = "asc"
                ...
                if (yOrder === "asc") {
                    ...
                    var iYRange = oYPoints[aBox[1]].getHigh() - oYPoints[aBox[1]].getLow();
                    var iScrStepY = iYRange / iStepSize;
                    i = 0;
                    ...
                    if (iScrY === "undefined" || isNaN(iScrY)) {
                        sTempScrY = oYPoints[0].getLow();
                    }
                    if (iScrY > oYPoints[oYPoints.length - 1].getHigh()) {
                        sTempScrY = oYPoints[oYPoints.length - 1].getHigh();
                    } else if (iScrY < oYPoints[0].getLow()) {
                        sTempScrY = oYPoints[0].getLow();
                    }

                    $.each(oYPoints, function (iDx, oValue) {
                        if (oValue && sTempScrY >= oValue.getLow()) {
                            aBox[1] = iDx;
                        }
                    });
                    // var iYRange = oYPoints[aBox[1]].getHigh() - oYPoints[aBox[1]].getLow();
                    ...
                    if (oCells) {
                        ...
                        for (var i in oCells) {
                            if (oCells[i].getIndex() === idx) {
                                ...
                                $rect.setAttribute("id",oCells[i].getIndex());
                                // if($text && oCells[i].getText()) {
                                //     $text.textContent = oCells[i].getText();
                                // }
                                // Update text with tspans
                                var cellText = oCells[i].getText();
                                if ($text && cellText) {
                                    ...
                                    if (match) {
                                        ...
                                        for (var j = 0; j < cellText.length; j += maxLen) {
                                            lines.push(cellText.substring(j, j + maxLen));
                                        }
                                    }
  
                                    var offsetY = -((lines.length - 1) * 12) / 2;
                                    for (var k = 0; k < lines.length; k++) {
                                        var tspan = document.createElementNS("http://www.w3.org/2000/svg", "tspan");
                                        tspan.setAttribute("x", $text.getAttribute("x"));
                                        tspan.setAttribute("dy", k === 0 ? offsetY : "1.2em");
        ...
        renderer: function (oRm, oControl) {
            ...
            if (oControl.getAggregation("yAxisData") !== null && typeof oControl.getAggregation("yAxisData")[1] !== "undefined") {
                ...
                iSvgWidth = (oControl.getCols() + 2.3) * 100;
            }
